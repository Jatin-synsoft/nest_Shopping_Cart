import { Body, Controller, Get, Post, Headers, Put, Query, UsePipes, ValidationPipe, Render } from '@nestjs/common';
...
export class UserController {
  ...
  @Get('/')
  @Render('product')
  async showAllProducts(@Query('name') name: string, @Query('price') price: number): Promise<any> {
    const products = await this.userservice.showallProdcuts(name, price);
    return {products}
  }
